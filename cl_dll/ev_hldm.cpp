...
void EV_FireShotGunDouble(event_args_t* args)
{
	...
	*/
	...
	{
		EV_HLDM_FireBullets(idx, forward, right, up, 12, vecSrc, vecAiming, 2048, BULLET_PLAYER_BUCKSHOT, 0, &tracerCount[idx - 1], 0.06105, 0.06105);
	}
}
...
void EV_FireShotGunSingle(event_args_t* args)
{
	...
	{
		EV_HLDM_FireBullets(idx, forward, right, up, 6, vecSrc, vecAiming, 2048, BULLET_PLAYER_BUCKSHOT, 0, &tracerCount[idx - 1], 0.04362, 0.04362);
	}
}
//======================
//	   SHOTGUN END
...
void EV_FireMP5(event_args_t* args)
{
	...
	EV_GetGunPosition(args, vecSrc, origin);
	EV_HLDM_MuzzleFlash(vecSrc, 0.5 + gEngfuncs.pfnRandomFloat(-0.2, 0.2));
	VectorCopy(forward, vecAiming);
...
void EV_FireM162(event_args_t* args)
{
	...
	if (EV_IsLocal(idx))
	{
		gEngfuncs.pEventAPI->EV_WeaponAnimation(M16_GRENADE, 0);
		V_PunchAxis(0, -10);
	}
...
void EV_FireGauss(event_args_t* args)
{
	...
	m_iBeam = gEngfuncs.pEventAPI->EV_FindModelIndex("sprites/xbeam1.spr");
	m_iBalls = m_iGlow = gEngfuncs.pEventAPI->EV_FindModelIndex("sprites/XSpark1.spr");
...
void EV_HornetGunFire(event_args_t* args)
{
	...
	shell = gEngfuncs.pEventAPI->EV_FindModelIndex("models/762shell.mdl"); // brass shell
	...
	{
	case 0:
		gEngfuncs.pEventAPI->EV_PlaySound(idx, origin, CHAN_WEAPON, "hassault/hw_shoot1.wav", 1, ATTN_NORM, 0, 94 + gEngfuncs.pfnRandomLong(0, 0xf));
		break;
	case 1:
		gEngfuncs.pEventAPI->EV_PlaySound(idx, origin, CHAN_WEAPON, "hassault/hw_shoot2.wav", 1, ATTN_NORM, 0, 94 + gEngfuncs.pfnRandomLong(0, 0xf));
		break;
	case 2:
		gEngfuncs.pEventAPI->EV_PlaySound(idx, origin, CHAN_WEAPON, "hassault/hw_shoot3.wav", 1, ATTN_NORM, 0, 94 + gEngfuncs.pfnRandomLong(0, 0xf));
		break;
	}
	...
	EV_HLDM_FireBullets(idx, forward, right, up, 1, vecSrc, vecAiming, 8192, BULLET_PLAYER_762, 2, &tracerCount[idx - 1], args->fparam1, args->fparam2);
}
//======================
//	   HORNET END
//======================
...
void EV_HLDM_Particles(vec_t Pos_X, vec_t Pos_Y, vec_t Pos_Z, float PosNorm_X, float PosNorm_Y, float PosNorm_Z, int DoPuff, int Material)
{
	...
	if (Material == 0)//concrete, tile
	{
		...
		*/
		fDoPuffs = true;
		fDoSparks = (gEngfuncs.pfnRandomLong(1, 3) == 1);
		fDoMuzzle = false;
		fDoParticles = true;
		a = 96;
		r = 128;
	...
	if (DoPuff != 0)
	{
		if (fDoPuffs)
		{ // get sprite index
			int  iWallsmoke = gEngfuncs.pEventAPI->EV_FindModelIndex("sprites/gunsmoke.spr");
	...
	if (fDoMuzzle)
	{// draw the dynamic light (thanks to ToMMuk)
		dlight_t *dl = gEngfuncs.pEfxAPI->CL_AllocDlight(0);
	...
	float NumParticles = gEngfuncs.pfnRandomFloat(1, 3);
