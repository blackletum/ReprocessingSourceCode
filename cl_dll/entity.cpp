...
void SmokeEffect(int attachment, const struct cl_entity_s* entity)
{
	...
	if (iSmoke != 0)
	{
		pSmokeTempEntity = gEngfuncs.pEfxAPI->R_TempSprite((float *)&entity->attachment[attachment], Vector(1, 1, 1)/*forward * gEngfuncs.pfnRandomLong ( -1 , 1 ) + right * gEngfuncs.pfnRandomLong ( -1 , 1 ) + up * gEngfuncs.pfnRandomLong ( -1 , 1 )*/,
		...
		{    // sprite created successfully, adjust some things
			pSmokeTempEntity->fadeSpeed = gEngfuncs.pfnRandomLong(3.5, 4.5);
			pSmokeTempEntity->entity.curstate.renderamt = 255;
			pSmokeTempEntity->entity.curstate.rendercolor.r = 30;
			pSmokeTempEntity->entity.curstate.rendercolor.g = 30;
			pSmokeTempEntity->entity.curstate.rendercolor.b = 30;
		}
	}
	//*** SMOKE ****
...
void DLLEXPORT HUD_StudioEvent(const struct mstudioevent_s* event, const struct cl_entity_s* entity)
{
	...
	case 5004:
		...
		// Eject a brass shell
	case 6001:
		EjectBrass((float *)&entity->attachment[1], entity->angles, entity->angles, atoi(event->options), EV_IsLocal(entity->index));
		break;
	default:
		break;
	}
