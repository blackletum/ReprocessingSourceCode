...
#define AMMO_PICKUP_HEIGHT_MAX (gHUD.m_iHudScaleWidth - 100)
	...
	for (int i = 0; i < MAX_HISTORY; i++)
	{
		if (HISTSLOT_EMPTY != rgAmmoHistory[i].type)
		{
			...
			else if (rgAmmoHistory[i].type == HISTSLOT_AMMO)
			{
				...
				int ypos = gHUD.m_iHudScaleHeight - (AMMO_PICKUP_PICK_HEIGHT + (AMMO_PICKUP_GAP * i));
				int xpos = gHUD.m_iHudScaleWidth - 24;
			...
			else if (rgAmmoHistory[i].type == HISTSLOT_WEAP)
			{
				...
				int ypos = gHUD.m_iHudScaleHeight - (AMMO_PICKUP_PICK_HEIGHT + (AMMO_PICKUP_GAP * i));
				int xpos = gHUD.m_iHudScaleWidth - (weap->rcInactive.right - weap->rcInactive.left);
			...
			else if (rgAmmoHistory[i].type == HISTSLOT_ITEM)
			{
				...
				int ypos = gHUD.m_iHudScaleHeight - (AMMO_PICKUP_PICK_HEIGHT + (AMMO_PICKUP_GAP * i));
				int xpos = gHUD.m_iHudScaleWidth - (rect.right - rect.left) - 10;
