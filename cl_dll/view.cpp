...
extern cvar_t* cl_viewlag;

#define CAM_MODE_RELAX 1
#define CAM_MODE_FOCUS 2

Vector v_origin, v_angles, v_cl_angles, v_sim_org, v_lastAngles, v_sim_vel;
...
void V_CalcBob2(struct ref_params_s *pparams, float freqmod, calcBobMode_t mode, double &bobtime, float &bob, float &lasttime)
{
	float	cycle;
	Vector	vel;
...
float blendMouseX = 0.0;
float blendMouseY = 0.0;
...
void V_CalcNormalRefdef(struct ref_params_s* pparams)
{
	...
	// transform the view offset by the model's matrix to get the offset from
	// model origin for the view
	V_CalcBob2(pparams, 0.75f, VB_SIN, bobtimes[0], bobRight, lasttimes[0]); // right
	V_CalcBob2(pparams, 1.50f, VB_SIN, bobtimes[1], bobUp, lasttimes[1]); // up
	V_CalcBob2(pparams, 1.00f, VB_SIN, bobtimes[2], bobForward, lasttimes[2]); // forward
	...
	//cvar_t* m_rawinput = gEngfuncs.pfnGetCvarPointer("m_rawinput");
	...
	float relMouseX = (float)(mouseX) * 0.1;
	float relMouseY = (float)(mouseY) * 0.1;

	if (relMouseX > 5.0)
		relMouseX = 5.0;
	else if (relMouseX < -4.0)
		relMouseX = -4.0;

	if (relMouseY > 4.0)
		relMouseY = 4.0;
	else if (relMouseY < -5.0)
		relMouseY = -5.0;
	...
	if (0 == cl_viewlag->value)
	{
		blendMouseX = 0;
		blendMouseY = 0;
	}

	//blendMouseX = blendMouseX * 0.95;
	//blendMouseY = blendMouseY * 0.95;
	if (blendMouseX < 0.01 && blendMouseX > -0.01)
		blendMouseX = 0;
	if (blendMouseY < 0.01 && blendMouseY > -0.01)
		blendMouseY = 0;
	...
	if (view->model)
	{
		...
		for (int i = 0; i < 3; i++)
		{
				...
				)
			{
				...
				//gHUD.m_flCrosshairOffsetX = blendMouseX;
				//gHUD.m_flCrosshairOffsetY = blendMouseY;
			}
		}
	}
	pparams->viewangles[ROLL] += bobRight * 0.10;
	pparams->viewangles[PITCH] += bobRight * 0.20;
