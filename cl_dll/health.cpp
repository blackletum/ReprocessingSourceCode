...
bool CHudHealth::Draw(float flTime)
{
	...
	if (true)
	{
		...
		y = gHUD.m_iHudScaleHeight - gHUD.m_iFontHeight - gHUD.m_iFontHeight / 2;
		x = CrossWidth / 2;
		...
		{
			UnpackRGB(r, g, b, RGB_YELLOWISH);
			y = gHUD.m_iHudScaleHeight - gHUD.m_iFontHeight - gHUD.m_iFontHeight * 2.5;
			x = CrossWidth / 2;

			SPR_Set(gHUD.GetSprite(m_HUD_card), r, g, b);
			SPR_DrawAdditive(0, x, y, &gHUD.GetSpriteRect(m_HUD_card));

			if (gHUD.m_iNumCards > 1)
			{
				y = gHUD.m_iHudScaleHeight - gHUD.m_iFontHeight - gHUD.m_iFontHeight * 2;
				x = CrossWidth + HealthWidth * 2.5;
...
bool CHudHealth::DrawDamage(float flTime)
{
	...
	for (i = 0; i < NUM_DMG_TYPES; i++)
	{
		...
		if ((m_bitsDamage & giDmgFlags[i]) != 0)
		{
			if (giDmgFlags[i] == DMG_NERVEGAS)
			{
				pdmg->fExpire = V_min(flTime + 4, pdmg->fExpire);
			}
			else
...
void CHudHealth::UpdateTiles(float flTime, long bitsDamage)
{
	...
	for (int i = 0; i < NUM_DMG_TYPES; i++)
	{
		...
		if ((bitsOn & giDmgFlags[i]) != 0)
		{
			...
			pdmg->x = giDmgWidth / 8;
			pdmg->y = gHUD.m_iHudScaleHeight - giDmgHeight * 2.5;
			if (giDmgFlags[i] == DMG_NERVEGAS)
			{
				pdmg->fExpire = flTime + 4;
				m_bNerve = true;
