...
int __MsgFunc_CardHud(const char* pszName, int iSize, void* pbuf)
{
	return static_cast<int>(gHUD.MsgFunc_CardHud(pszName, iSize, pbuf));
}
...
void CHud::Init()
{
	...
	HOOK_MESSAGE(Impact);//Ku2zoff

	HOOK_MESSAGE(CardHud);
	...
	cl_viewlag = CVAR_CREATE("cl_viewlag", "1", FCVAR_ARCHIVE);
...
bool CHud::MsgFunc_CardHud(const char* pszName, int iSize, void* pbuf)
{
	BEGIN_READ(pbuf, iSize);

	m_iNumCards = READ_BYTE();
