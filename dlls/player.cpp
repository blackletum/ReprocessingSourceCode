...
TYPEDESCRIPTION CBasePlayer::m_playerSaveData[] =
	{
		...
		DEFINE_FIELD(CBasePlayer, m_iTrack, FIELD_INTEGER), // mp3 playmemory
...
bool CBasePlayer::TakeHealth(float flHealth, int bitsDamageType)
{
	...
	m_bitsDamageType &= DMG_RADIATION;
	m_rgbTimeBasedDamage[itbd_Poison] = 0;
	m_rgbTimeBasedDamage[itbd_Acid] = 0;
...
bool CBasePlayer::ShouldCollide(CBaseEntity *pOther)
{
	//if (pOther->pev->deadflag == DEAD_DEAD && FBitSet(pOther->pev->flags, FL_MONSTER))
	//	return false;
	return true;
}
...
const char* GetTrack(int iTrack)
{
	switch (iTrack)
	{
	case 1:
		return "Reprocessing.mp3";
		break;
	case 2:
		return "Half-Life01.mp3";
		break;
	case 3:
		return "Prospero01.mp3";
		break;
	case 4:
		return "Half-Life12.mp3";
		break;
	case 5:
		return "Half-Life07.mp3";
		break;
	case 6:
		return "Half-Life10.mp3";
		break;
	case 7:
		return "Suspense01.mp3";
		break;
	case 8:
		return "Suspense03.mp3";
		break;
	case 9:
		return "Half-Life09.mp3";
		break;
	case 10:
		return "Half-Life02.mp3";
		break;
	case 11:
		return "Half-Life13.mp3";
		break;
	case 12:
		return "Half-Life04.mp3";
		break;
	case 13:
		return "Half-Life15.mp3";
		break;
	case 14:
		return "Half-Life14.mp3";
		break;
	case 15:
		return "Half-Life16.mp3";
		break;
	case 16:
		return "Suspense02.mp3";
		break;
	case 17:
		return "Half-Life03.mp3";
		break;
	case 18:
		return "Half-Life08.mp3";
		break;
	case 19:
		return "Prospero02.mp3";
		break;
	case 20:
		return "Half-Life05.mp3";
		break;
	case 21:
		return "Prospero04.mp3";
		break;
	case 22:
		return "Half-Life11.mp3";
		break;
	case 23:
		return "Half-Life06.mp3";
		break;
	case 24:
		return "Prospero03.mp3";
		break;
	case 25:
		return "Half-Life17.mp3";
		break;
	case 26:
		return "Prospero05.mp3";
		break;
	case 27:
		return "Suspense05.mp3";
		break;
	case 28:
		return "Suspense07.mp3";
		break;
	case 29:
		return "gamestartup.mp3";
		break;
	default:
		return "not_found.mp3";
		break;
	}
}

void RestoreCDTrack(int iTrack)
{
...
bool CBasePlayer::Restore(CRestore& restore)
{
	...
	if (!strcmp(STRING(gpGlobals->mapname), pSaveData->szCurrentMapName))
		m_bRestoreTrack = true;
	else
		m_bRestoreTrack = false;
...
void CBasePlayer::UpdateClientData()
{
	...
	if (m_bRestored)
	{
		...
		{
			ALERT(at_console, "\n\n replay track = %d\n\n", m_iTrack);
			if (m_iTrack >= -1 && m_iTrack <= 30)
				RestoreCDTrack(m_iTrack);
		}
	}
	...
	MESSAGE_BEGIN(MSG_ONE, gmsgCard, NULL, pev);
	WRITE_BYTE(m_rgItems[ITEM_SECURITY]);
	MESSAGE_END();
...
LINK_ENTITY_TO_CLASS(player_weapondelete, CKillWeapon);

TYPEDESCRIPTION CKillWeapon::m_SaveData[] =
{
	DEFINE_FIELD(CKillWeapon, m_iWeapon, FIELD_STRING),
};

IMPLEMENT_SAVERESTORE(CKillWeapon, CPointEntity);

bool CKillWeapon::KeyValue(KeyValueData* pkvd)
{
	if (FStrEq(pkvd->szKeyName, "weapon"))
	{
		//ALERT(at_console, "CKillWeapon:Get val %s\n", pkvd->szValue);
		m_iWeapon = ALLOC_STRING(pkvd->szValue);
		return true;
	}
...
void CKillWeapon::Use(CBaseEntity* pActivator, CBaseEntity* pCaller, USE_TYPE useType, float value)
{
	//ALERT(at_console, "CKillWeapon:Try to kill\n");
	if (m_iWeapon == NULL)
		return;
	//ALERT(at_console, "CKillWeapon:String is exists and it's = %s\n", STRING(m_iWeapon));
	CBasePlayer* pPlayer = NULL;
	...
	if (pPlayer)
	{
		//ALERT(at_console, "CKillWeapon:String and player is exists. Search for weapon\n");
		...
		for (int i = 0; i < MAX_ITEM_TYPES; i++)
		{
			...
			while (pItem)
			{
				//ALERT(at_console, "CKillWeapon:Iterate:%s compare to %s\n", STRING(pItem->pev->classname), STRING(m_iWeapon));
				if (!strcmp(STRING(pItem->pev->classname), STRING(m_iWeapon)))
				{
					//ALERT(at_console, "CKillWeapon:Item fund. Try to kill\n");
					if (pPlayer->m_pActiveItem == pItem)
					{
						pPlayer->ResetAutoaim();
						pItem->Holster();
						pItem->pev->nextthink = 0; // crowbar may be trying to swing again, etc.
						pItem->SetThink(NULL);
					...
					if (pPlayer->m_pLastItem == pItem)
						pPlayer->m_pLastItem = NULL;

					CBasePlayerItem* pPrev = pPlayer->m_rgpPlayerItems[i];

					if (pPrev == pItem)
					{
						pPlayer->m_rgpPlayerItems[i] = pItem->m_pNext;
					}
					else
					{
						while (pPrev && pPrev->m_pNext != pItem)
					...
					pPlayer->ClearWeaponBit(pItem->m_iId); // take item off hud
...
class CUseCard : public CPointEntity
{
public:
	void Use(CBaseEntity* pActivator, CBaseEntity* pCaller, USE_TYPE useType, float value) override;
};

LINK_ENTITY_TO_CLASS(player_usecard, CUseCard);

void CUseCard::Use(CBaseEntity* pActivator, CBaseEntity* pCaller, USE_TYPE useType, float value)
{
	CBasePlayer* pPlayer = NULL;
	...
	if (pPlayer)
	{
		if (pPlayer->m_rgItems[ITEM_SECURITY])
			pPlayer->m_rgItems[ITEM_SECURITY]--;
		if (pPlayer->m_rgItems[ITEM_SECURITY] < 0)
			pPlayer->m_rgItems[ITEM_SECURITY] = 0;
		pPlayer->UpdateClientData();
	}
}
