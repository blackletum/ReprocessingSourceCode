...
void COsprey::FindAllThink()
{
	...
	if (m_iUnits == 0 && (pev->spawnflags & SF_MONSTER_PREDISASTER) == 0)
	{
		ALERT(at_console, "osprey error: no grunts to resupply\n");
		UTIL_Remove(this);
		return;
	}
	else if ((pev->spawnflags & SF_MONSTER_PREDISASTER) != 0)
	{
		m_iUnits = MAX_CARRY;
	}
...
CBaseMonster* COsprey::MakeGrunt(Vector vecSrc)
{
	...
	for (int i = 0; i < m_iUnits; i++)
	{
		if (m_hGrunt[i] == NULL || !m_hGrunt[i]->IsAlive() || (pev->spawnflags & SF_MONSTER_PREDISASTER) != 0)
		{
			if (m_hGrunt[i] != NULL && m_hGrunt[i]->pev->rendermode == kRenderNormal && (pev->spawnflags & SF_MONSTER_PREDISASTER) == 0)
			{
