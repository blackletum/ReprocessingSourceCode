...
#define		ISLAVE_EYE_FRAMES 2 // how many different switchable maps for the eye
	...
	case ISLAVE_AE_ZAP_SHOOT:
	{
		...
		if (m_hDead != NULL)
		{
			...
			if (0 == trace.fStartSolid)
			{
				...
				CISlave *pSlave = (CISlave *)Instance(pNewMonster->edict());
				CISlave *pOldSlave = (CISlave *)Instance(m_hDead->edict());
				pSlave->pev->sequence = m_hDead->pev->sequence;
				pSlave->m_Activity = pOldSlave->m_Activity;
				pSlave->pev->frame = m_hDead->pev->frame;
				//pSlave->pev->spawnflags |= 1;
				pSlave->pev->health = gSkillData.slaveHealth / 1.5;
				pSlave->m_iDeadseq = pOldSlave->m_iDeadseq;
				pSlave->Revive();
				WackBeam(-1, pNew);
				WackBeam(1, pNew);
				UTIL_Remove(m_hDead);
	...
	while ((pEntity = UTIL_FindEntityByClassname(pEntity, "monster_alien_slave")) != NULL)
	{
		...
		if (tr.flFraction == 1.0 || tr.pHit == pEntity->edict())
		{
			if (pEntity->pev->deadflag == DEAD_DEAD)
			{
				...
				if (d < flDist)
				{
					CISlave *pSlave = (CISlave *)Instance(pEntity->edict());
					if (pSlave->m_iDeadseq < g_iSkillLevel - 1)
					{
						m_hDead = pEntity;
						flDist = d;
					}
...
void CISlave::PrescheduleThink(void)
{
	// at random, initiate a blink if not already blinking or sleeping
	if ((pev->skin == 0) && RANDOM_LONG(0, 0X7) == 0)
	{// start blinking!
		pev->skin = 1;
		m_fDontBlink = 0;
	}
	else if (pev->skin == 1 && m_fDontBlink == 0)
	{// already blinking
		pev->skin = 2;
		m_fDontBlink = 1;
	}
	else if (pev->skin == 2 && m_fDontBlink == 1)
	{// already blinking
		pev->skin = 1;
	}
	else if (pev->skin == 1 && m_fDontBlink == 1)
	{// already blinking
		pev->skin = 0;
		m_fDontBlink = 0;
	}
...
void CISlave::Revive()
{
	...
	StudioFrameAdvance();
	ALERT(at_console, "CIslave: Reviving!\n");

	if (!m_bReviving)
	{
		pev->frame = 0;
		SetThink(&CISlave::Revive);
		m_bReviving = true;
		m_iDeadseq++;

		if (m_Activity == ACT_DIEFORWARD || pev->sequence == LookupSequence("dieviolent2"))
		{
			pev->framerate = 2.0;
			pev->sequence = LookupSequence("RES-Die(Forward)");
		}
		else if (m_Activity == ACT_DIEBACKWARD || pev->sequence == LookupSequence("dieviolent3"))
		{
			pev->framerate = 4.0;
			pev->sequence = LookupSequence("RES-Die(Bwrd)1");
		}
		else if (m_Activity == ACT_DIE_HEADSHOT)
		{
			pev->framerate = 2.0;
			pev->sequence = LookupSequence("RES-Die(Headshot)");
		}
		else
		{
			pev->framerate = 2.0;
			pev->sequence = LookupSequence("RES-Die(Simple)");
		}
		ResetSequenceInfo();
	}
	...
	{
		SetThink(&CISlave::MonsterInitThink);
	}
}