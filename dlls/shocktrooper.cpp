...
void CShockBeam::BallTouch(CBaseEntity* pOther)
{
	...
	if (pOther->pev->takedamage != DAMAGE_NO)
	{
		...
		const auto damage = 8; // gSkillData.plrDmgShockRoachM : gSkillData.plrDmgShockRoach;
...
void CSpore::Spawn()
{
	...
	pev->dmg = 50;//gSkillData.plrDmgSpore;
...
void CSpore::IgniteThink()
{
	...
	WRITE_COORD(pev->origin.y);
	WRITE_COORD(pev->origin.z);
	WRITE_COORD(pev->velocity.Normalize().x);
	WRITE_COORD(pev->velocity.Normalize().y);
	WRITE_COORD(pev->velocity.Normalize().z);
...
void CSpore::RocketTouch(CBaseEntity* pOther)
{
	if (pOther->pev->takedamage != DAMAGE_NO)
	{
		pOther->TakeDamage(pev, VARS(pev->owner), 50, DMG_GENERIC); //gSkillData.plrDmgSpore
	}
