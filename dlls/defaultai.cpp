...
Schedule_t* CBaseMonster::GetScheduleOfType(int Type)
{
	...
	{
		// freeslave
		if (HasMemory(bits_MEMORY_BLOCKER_IS_ENEMY) && IsRusher())
			return GetScheduleOfType(SCHED_CHASE_ENEMY);
		else if (HasMemory(bits_MEMORY_FORCED_TO_ATTACK) && IsRusher())
		{
			Forget(bits_MEMORY_FORCED_TO_ATTACK);
			if ((m_afCapability & bits_CAP_MELEE_ATTACK1) != 0)
				return GetScheduleOfType(SCHED_MELEE_ATTACK1);
			else if ((m_afCapability & bits_CAP_RANGE_ATTACK1) != 0)
				return GetScheduleOfType(SCHED_RANGE_ATTACK1);
			else if ((m_afCapability & bits_CAP_MELEE_ATTACK2) != 0)
				return GetScheduleOfType(SCHED_MELEE_ATTACK2);
			else if ((m_afCapability & bits_CAP_RANGE_ATTACK2) != 0)
