...
//===================================================================================
// This function calls impact effects based on texture type 
//===================================================================================
...
void CBaseMonster::GibMonster()
{
	...
	Vector center = pev->origin;
	center.z = center.z + (UTIL_GetModelHeight(pev));

	SpawnBlood(center, m_bloodColor, UTIL_GetModelMaxDim(pev));

	center = pev->origin;
	center.z = center.z + (UTIL_GetModelHeight(pev) / 2);

	SpawnBlood(center, m_bloodColor, UTIL_GetModelMaxDim(pev) * 2);
...
================
*/
void SendEffects(Vector vecTracerSrc, float r, float g, float b, float radius)
{
	MESSAGE_BEGIN(MSG_PVS, SVC_TEMPENTITY, vecTracerSrc);
	WRITE_BYTE(TE_DLIGHT);
	WRITE_COORD(vecTracerSrc.x);			 // X
	WRITE_COORD(vecTracerSrc.y);			 // Y
	WRITE_COORD(vecTracerSrc.z);			 // Z
	WRITE_BYTE(radius * 10);					 // radius * 0.1
	WRITE_BYTE(r);				 // r
	WRITE_BYTE(g);				 // g
	WRITE_BYTE(b);					 // b
	WRITE_BYTE(0.01); // time * 10
...
void CBaseEntity::FireBullets(unsigned int cShots, Vector vecSrc, Vector vecDirShooting, Vector vecSpread, float flDistance, int iBulletType, int iTracerFreq, int iDamage, entvars_t* pevAttacker)
{
	...
	for (unsigned int iShot = 1; iShot <= cShots; iShot++)
	{
		...
		if (iTracerFreq != 0 && (tracerCount++ % iTracerFreq) == 0)
		{
			...
			float r = 255, g = 255, b = 128, dlradius;

			switch (iBulletType)
			{
			case BULLET_MONSTER_12MM:
				dlradius = 1.5 + RANDOM_FLOAT(-0.2, 0.2);
				break;
			case BULLET_MONSTER_MP5:
				dlradius = 0.5 + RANDOM_FLOAT(-0.2, 0.2);
				break;
			case BULLET_MONSTER_M16:
				dlradius = 1.5 + RANDOM_FLOAT(-0.2, 0.2);
				break;
			case BULLET_MONSTER_9MM:
				if (FClassnameIs(pevAttacker, "monster_human_assassin"))
				{
					dlradius = 0.5 + RANDOM_FLOAT(-0.2, 0.2);
					break;
				}
			case BULLET_MONSTER_357:
			default:
				dlradius = 1.0 + RANDOM_FLOAT(-0.2, 0.2);
				break;
			}
			SendEffects(vecSrc, r, g, b, dlradius);
		...
		if (tr.flFraction != 1.0)
		{
			...
			if (0 != iDamage)
			{
				...
				case BULLET_MONSTER_M16:
					pEntity->TraceAttack(pevAttacker, gSkillData.monDmgM16, vecDir, &tr, DMG_BULLET);
