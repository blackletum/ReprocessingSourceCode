...
void CStomp::Think()
{
	...
	// Freeslave Fix Garg not being able to attack enemy with a stomp attack when enemy is hiding in the niche so Garg can't see it
	...
	*/
	auto FindSomethingAndDoDamage = [&]() -> CBaseEntity* {
		...
		CBaseEntity *pEntity = CBaseEntity::Instance(tr.pHit);
			if (pEntity && pEntity->pev->takedamage)
			{
				...
				pEntity->TakeDamage(pev, pevAttacker, gSkillData.gargantuaDmgStomp, DMG_SONIC);
				return pEntity;
			}
		}
		return nullptr;
	...
	while (gpGlobals->time - pev->dmgtime > STOMP_INTERVAL)
	{
		...
		for (int i = 0; i < 2; i++)
		{
			...
			CSprite *pSprite = CSprite::SpriteCreate(GARG_STOMP_SPRITE_NAME, tr.vecEndPos, true);
			if (pSprite)
			{
				//UTIL_TraceLine(pev->origin, pev->origin - Vector(0, 0, 500), ignore_monsters, edict(), &tr);
