...
#include "player.h" //Это понадобится нам для задекларирования новых определений

#define MONSTER_CUT_CORNER_DIST 8 // 8 means the monster's bounding box is contained without the box of the node in WC

#include "pm_shared.h"//Это понадобится нам для определения типов текстур
...
TYPEDESCRIPTION CBaseMonster::m_SaveData[] =
	{
		...
		DEFINE_FIELD(CBaseMonster, m_hOverEnemy, FIELD_EHANDLE),
		...
		DEFINE_FIELD(CBaseMonster, m_Gait, FIELD_INTEGER),
		DEFINE_FIELD(CBaseMonster, m_IdealGait, FIELD_INTEGER),
		#endif // GATESEQ
};
...
static TYPEDESCRIPTION gAIExtendedSaveHeader[] =
{
	DEFINE_FIELD(AIExtendedSaveHeader_t, version,			FIELD_SHORT),
	DEFINE_FIELD(AIExtendedSaveHeader_t, flags,			FIELD_INTEGER),
	DEFINE_ARRAY(AIExtendedSaveHeader_t, szSchedule,	FIELD_CHARACTER, 128),
	DEFINE_FIELD(AIExtendedSaveHeader_t, scheduleCrc,		FIELD_INTEGER),
	DEFINE_ARRAY(AIExtendedSaveHeader_t, szSequence,	FIELD_CHARACTER, 128),
};
...
bool CBaseMonster::Save(CSave& save)
{
	...
	if (m_pSchedule)
	{
		const char *pszSchedule = m_pSchedule->pName;
		...
		CRC32_PROCESS_BUFFER(&saveHeader.scheduleCrc, (void *)m_pSchedule->pTasklist, m_pSchedule->cTasks * sizeof(Task_t));
	...
	if (pev->sequence != ACTIVITY_NOT_AVAILABLE && GET_MODEL_PTR(edict()))
	{
		...
		if (pszSequenceName && *pszSequenceName)
		{
			...
			saveHeader.szSequence[sizeof(saveHeader.szSequence) - 1] = 0;
		}
	}

	save.WriteFields("AIExtendedSaveHeader", &saveHeader, gAIExtendedSaveHeader, ARRAYSIZE(gAIExtendedSaveHeader));
...
void CBaseMonster::DiscardScheduleState()
{
	...
	if (bLostScript)
	{
		...
		ALERT(at_console, "Scripted Sequence stripped on level transition for %s\n", STRING(pev->targetname));
	}
}
...
bool CBaseMonster::Restore(CRestore& restore)
{
	...
	AIExtendedSaveHeader_t saveHeader;
	restore.ReadFields("AIExtendedSaveHeader", &saveHeader, gAIExtendedSaveHeader, ARRAYSIZE(gAIExtendedSaveHeader));

	bool status = restore.ReadFields("CBaseMonster", this, m_SaveData, ARRAYSIZE(m_SaveData));

	bool bLostSequence = false;
	if (saveHeader.szSequence[0] && GET_MODEL_PTR(edict()))
	{
	...
	bool bDiscardScheduleState = (bLostScript ||
		...
		((saveHeader.flags & AIESH_HAD_ENEMY) && !m_hEnemy) ||
		((saveHeader.flags & AIESH_HAD_TARGET) && !m_hTargetEnt));

	if (!bDiscardScheduleState)
	{
		m_pSchedule = ScheduleFromName(saveHeader.szSchedule);
		if (m_pSchedule)
		{
			CRC32_t scheduleCrc;
			CRC32_INIT(&scheduleCrc);
			CRC32_PROCESS_BUFFER(&scheduleCrc, (void *)m_pSchedule->pTasklist, m_pSchedule->cTasks * sizeof(Task_t));
...
bool CBaseMonster::FRefreshRoute()
{
	...
	case MOVEGOAL_PATHCORNER:
	{
		...
		// Freeslave code for move to nearest
	case MOVEGOAL_ENEMY:
	case MOVEGOAL_ENEMY_NEAREST:
		returnCode = BuildRoute(m_vecEnemyLKP, m_movementGoal, m_hEnemy);
		break;
		...
		if (!FStringNull(pcbeDoor->pev->targetname))
		{
			...
			for (;;)
			{
				...
				if (VARS(pentTarget) != pcbeDoor->pev)
				{
					...
					{
						// Need to cast door because we need to check door state
						CBaseDoor* pDoor = (CBaseDoor*)Instance(pentTarget);
						if (pDoor)
						{
							// Freeslave inspired idea of fix monsters and wait on opened or closed doors closed for monsters
							if (pDoor->m_toggle_state == TS_AT_TOP)
								flTravelTime = 0;
							else if (FBitSet(pDoor->pev->spawnflags, 512))
								flTravelTime = 0;
							else
								pDoor->Use(this, this, USE_ON, 0.0);
							// end of Freeslave inspired idea
							// Old idea - anyway use door and wait for them
							//pDoor->Use(this, this, USE_ON, 0.0);
						}
					}
				}
	...
	auto nearestBuildRoute = [&]()
	{
		if (nearest)
		{
			...
			else
			{
				...
				if ((localMoveNearest - pev->origin).Length2D() >= (1.0f))
				{
...
// Freeslave code for move to nearest New function
...
bool CBaseMonster::FTriangulate( const Vector &vecStart, const Vector &vecEnd, float flDist, CBaseEntity *pTargetEnt, Vector *pApex )
{
	...
	// Grab the size of the monster bounding box
	sizeX = pev->size.x * 0.5;
	sizeZ = pev->size.z * 0.5;

	vecForward = ( vecEnd - vecStart ).Normalize();

	// Compute a direction vector perpendicular to the desired motion direction
	Vector vecDirUp( 0, 0, 1 );
	vecDir = CrossProduct( vecForward, vecDirUp );	// Orthogonal to facing
	...
	// Compute triangulation apex points (MOVETYPE_FLY attempts vertical triangulation too)
	Vector vecDelta[2];
	Vector vecApex[4];

	Vector vecCenter;
	int nNumToTest = 2;
	vecDelta[0] = vecDir * sizeX;

	vecCenter = vecStart + ( vecForward * (flDist + sizeX) );
	vecApex[0] = vecCenter - vecDelta[0];
	vecApex[1] = vecCenter + vecDelta[0];
	vecDelta[0] = vecDelta[0] * 2;

	if (pev->movetype == MOVETYPE_FLY)
	{
		vecDelta[1] = vecDirUp * ( sizeZ * 3 );
		vecApex[2] = vecCenter - vecDelta[1];
		vecApex[3] = vecCenter + vecDelta[1];
		nNumToTest = 4;
	}
...
// Debug, Draw the triangulation
			...
			MESSAGE_BEGIN( MSG_BROADCAST, SVC_TEMPENTITY );
				...
				WRITE_COORD( vecApex[j].x );
				WRITE_COORD( vecApex[j].y );
				WRITE_COORD( vecApex[j].z );
			MESSAGE_END();
			...
			if ( CheckLocalMove( vecStart, vecApex[j], pTargetEnt, NULL ) == LOCALMOVE_VALID )
			{
				// Ok, we got from the start to the triangle apex, now try
				// the triangle apex to the end
				if ( CheckLocalMove( vecApex[j], vecEnd, pTargetEnt, NULL ) == LOCALMOVE_VALID )
				{
					*pApex = vecApex[j];
					return true;
				}
			...
			if (j & 0x1)
				vecApex[j] = vecApex[j] + vecDelta[j >> 1];
			else
				vecApex[j] = vecApex[j] - vecDelta[j >> 1];
		}
	}
		...
		else
		{
			...
			else
			{
				...
				else
				{
					...
					if (pBlocker && ( (FClassnameIs(pBlocker->pev, "func_breakable") && !FBitSet(pBlocker->pev->spawnflags, SF_BREAK_TRIGGER_ONLY))
						|| (FClassnameIs(pBlocker->pev, "func_pushable") && (pev->spawnflags & SF_PUSH_BREAKABLE) != 0) )
						&& IsRusher())
					{
						// UNDONE - Need to force monster to attack breakables
						//ALERT(at_aiconsole, "Scha uebu baricadu\n");
						//pBlocker->TakeDamage(pev,pev, gSkillData.zombieDmgBothSlash,DMG_CRUSH);
						Remember(bits_MEMORY_FORCED_TO_ATTACK);
					}
...
{
	if (FClassnameIs(pOther->pev, "player") && pOther->pev && pev->deadflag == DEAD_DEAD && FBitSet(pev->flags, FL_MONSTER))
		if (pOther->pev->button & IN_ATTACK || pOther->pev->button & IN_ATTACK2)
			return true;
		else
			return false;
	return true;
...
void CBaseMonster::MoveExecute(CBaseEntity* pTargetEnt, const Vector& vecDir, float flInterval)
{
	...
	if (FHaveGait())
	{
		if (m_IdealGait != m_movementActivity)
			m_IdealGait = m_movementActivity;
	}
	else
	{
		if (m_IdealActivity != m_movementActivity)
			m_actionActivity = m_movementActivity;
	}
#else
	if (m_IdealActivity != m_movementActivity)
...
#ifdef GATESEQ
	m_actionActivity = ACT_IDLE; // ku2zoff
#else
...
int CBaseMonster::IRelationship(CBaseEntity* pTarget)
{
	static int iEnemy[15][15] =
		{//   NONE	 MACH	 PLYR	 HPASS	 HMIL	 AMIL	 APASS	 AMONST	APREY	 APRED	 INSECT	PLRALY	PBWPN	ABWPN
			...
			/*HUMANPASSIVE*/ {R_NO, R_NO, R_AL, R_AL, R_HT, R_HT, R_NO, R_HT, R_DL, R_DL, R_NO, R_AL, R_NO, R_NO, R_NO},
			...
			/*RACE X*/ { R_NO ,R_NO ,R_DL ,R_DL ,R_DL ,R_AL ,R_NO ,R_DL ,R_DL ,R_NO ,R_NO ,R_DL, R_DL, R_NO, R_NO }
	};

	if (m_hOverEnemy && pTarget == m_hOverEnemy)
		return R_NM;
...
float NPC_Step(CBaseMonster *pMonster)
{
	...
	fvol = 1.0;//Это громкость шагов монстра
...
void CBaseMonster::HandleAnimEvent(MonsterEvent_t* pEvent)
{
	...
	case SCRIPT_EVENT_STEP: //ивент для динамических шагов
		//if (pEvent->options != NULL && pEvent->options[0])//проверка на наличие аргумента
		//	EMIT_SOUND(edict(), CHAN_BODY, pEvent->options, 1.0, ATTN_IDLE);//если в опцыях другой звук, то шаги нам не нужны
		NPC_Step(this);
		break;
...
void CBaseMonster::MaintainGait(void)
{
	/*	if (m_scriptState == SCRIPT_PLAYING || m_scriptState == SCRIPT_WAIT)
		{
			if (pev->gaitsequence != pev->sequence)
	...
	if (m_IdealActivity != m_actionActivity)
		m_IdealActivity = m_actionActivity;
	...
	if (!IsMoving())
	{
		if (m_IdealGait != m_actionActivity)
			m_IdealGait = m_actionActivity;
	}
	else
	{
		if (m_pLastSchedule && m_pLastSchedule != m_pSchedule)
		{
			m_pLastSchedule = NULL;

			if (m_IdealActivity != m_movementActivity)
				m_actionActivity = m_movementActivity;
		}
		else if (m_Activity == m_Gait && m_Activity != m_IdealGait)
		{
			m_actionActivity = m_IdealGait;
		}
	}

	if (m_IdealGait == GetDeathActivity())
		pev->gaitsequence = 255;

	if (pev->gaitsequence != 255)
	{
		void *pmodel = GET_MODEL_PTR(ENT(pev));
...
void CBaseMonster::SetGait(void)
{
	if (!FHaveGait())
		return;

	if (m_Gait != m_IdealGait)
	{
		int iGaitSequence = LookupActivity(m_IdealGait);
		...
		if (iGaitSequence > ACTIVITY_NOT_AVAILABLE)
		{
		...
		{
			ALERT(at_aiconsole, "%s has no gaitsequence for act:%d\n", STRING(pev->classname), m_IdealGait);
			pev->gaitsequence = 0;
		}
