...
inline void TransmitShakeEvent(CBaseEntity *pPlayer, float localAmplitude, float frequency, float duration)
{
	ScreenShake	shake;
...
void UTIL_ScreenShake(const Vector &center, float amplitude, float frequency, float duration, float radius)
{
	...
	for (i = 1; i <= gpGlobals->maxClients; i++)
	{
		...
		localAmplitude = ComputeShakeAmplitude(center, pPlayer->Center(), amplitude, radius);
		...
		TransmitShakeEvent(pPlayer, localAmplitude, frequency, duration);
	}
}
...
#define NUM_HULL_ROUNDS	(sizeof(hull_table) / sizeof(short))
#define HULL_PRECISION	4	// точность установки значения
short hull_table[] = { 0, 4, 8, 16, 18, 24, 28, 30, 32, 40, 48, 54, 56, 60, 64, 72, 80, 112, 120, 128, 140, 176 };

void UTIL_RoundHullSize(Vector size, bool down)
{
	int	i, j;
...
float UTIL_GetModelMaxDim(entvars_t* ent)
{
	studiohdr_t *pstudiohdr;
	pstudiohdr = (studiohdr_t*)GET_MODEL_PTR(ENT(ent));
	...
	return V_max(V_max(pseqdesc[ent->sequence].bbmax.z, pseqdesc[ent->sequence].bbmax.y), pseqdesc[ent->sequence].bbmax.x);
}

Vector UTIL_GetModelMinSize(entvars_t* ent)
{
	studiohdr_t *pstudiohdr;
	pstudiohdr = (studiohdr_t*)GET_MODEL_PTR(ENT(ent));
	...
	mstudioseqdesc_t    *pseqdesc;
	pseqdesc = (mstudioseqdesc_t *)((byte *)pstudiohdr + pstudiohdr->seqindex);
	return pseqdesc[ent->sequence].bbmin;
}

Vector UTIL_GetModelMaxSize(entvars_t* ent)
{
	studiohdr_t *pstudiohdr;
	pstudiohdr = (studiohdr_t*)GET_MODEL_PTR(ENT(ent));
	...
	mstudioseqdesc_t    *pseqdesc;
	pseqdesc = (mstudioseqdesc_t *)((byte *)pstudiohdr + pstudiohdr->seqindex);
	return pseqdesc[ent->sequence].bbmax;
}

float UTIL_GetModelHeight(entvars_t* ent)
{
	studiohdr_t *pstudiohdr;
	pstudiohdr = (studiohdr_t*)GET_MODEL_PTR(ENT(ent));
	...
	mstudioseqdesc_t    *pseqdesc;
	pseqdesc = (mstudioseqdesc_t *)((byte *)pstudiohdr + pstudiohdr->seqindex);
	return pseqdesc[ent->sequence].bbmax.z;
}

void UTIL_SetModelSize(entvars_t* ent)
{
	studiohdr_t *pstudiohdr;
	pstudiohdr = (studiohdr_t*)GET_MODEL_PTR(ENT(ent));
	...
	mstudioseqdesc_t    *pseqdesc;
	pseqdesc = (mstudioseqdesc_t *)((byte *)pstudiohdr + pstudiohdr->seqindex);
	UTIL_SetSize(ent, pseqdesc[ent->sequence].bbmin, pseqdesc[ent->sequence].bbmax);
}

void UTIL_SetSize(entvars_t* pev, const Vector& vecMin, const Vector& vecMax)
{
	UTIL_RoundHullSize(vecMin, false);	//округляем значение вверх, чтобы модель не ушла в пол
	UTIL_RoundHullSize(vecMax, false);
...
int CSaveRestoreBuffer::EntityIndex(CBaseEntity* pEntity)
{
	if (pEntity == NULL)
		return -1;
	const char* entname = STRING(pEntity->pev->classname);
...
void CSave::WriteFunction(const char* pname, void** data, int count)
{
	...
	if (functionName)
		...
		ALERT(at_error, "Invalid function %s pointer in entity!\n", pname);
}
...
{
	int index = gDecals[13].index;
	ALERT(at_console, "UTIL_StudioDecal");
