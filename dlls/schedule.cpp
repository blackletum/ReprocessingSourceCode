...
void CBaseMonster::MaintainSchedule()
{
	...
	for (i = 0; i < 10; i++)
	{
		...
		{
			if (TaskIsRunning() && !HasConditions(bits_COND_TASK_FAILED))
			{
				Task_t *pTask = GetTask();
				ASSERT(pTask != NULL);
				RunTask(pTask);
...
void CBaseMonster::SetTurnActivity()
{
	...
	{// big right turn
		m_actionActivity = ACT_TURN_RIGHT; // ku2zoff
	}
	else if (flYD > 45 && LookupActivity(ACT_TURN_LEFT) != ACTIVITY_NOT_AVAILABLE)
	{// big left turn
		m_actionActivity = ACT_TURN_LEFT; // ku2zoff
	}
#else
	if (flYD <= -45 && LookupActivity(ACT_TURN_RIGHT) != ACTIVITY_NOT_AVAILABLE)
	{ // big right turn
...
void CBaseMonster::StartTask(Task_t* pTask)
{
	...
	case TASK_STOP_MOVING:
	{
		...
		{
#ifdef GATESEQ
			m_actionActivity = GetStoppedActivity();
#else
		...
		if (FHaveGait())
		{
			if (m_IdealGait == m_movementActivity)
				m_IdealGait = GetStoppedActivity();
		}
		*/
#endif
	...
	{
#ifdef GATESEQ
		m_actionActivity = (Activity)(int)pTask->flData; // ku2zoff
#else
...
#ifdef GATESEQ
		m_actionActivity = (Activity)WorldGraph.m_pNodes[m_iHintNode].m_sHintActivity; // ku2zoff
#else
	...
	{
#ifdef GATESEQ
		m_actionActivity = ACT_MELEE_ATTACK1;
#else
	...
	{
#ifdef GATESEQ
		m_actionActivity = ACT_MELEE_ATTACK2;
#else
	...
	{
#ifdef GATESEQ
		m_actionActivity = ACT_RANGE_ATTACK1;
#else
	...
	{
#ifdef GATESEQ
		m_actionActivity = ACT_RANGE_ATTACK2;
#else
	...
	{
#ifdef GATESEQ
		m_actionActivity = ACT_SPECIAL_ATTACK1;
#else
	...
	{
#ifdef GATESEQ
		m_actionActivity = ACT_SPECIAL_ATTACK2;
#else
	...
	{
#ifdef GATESEQ
		m_actionActivity = (Activity)(int)pTask->flData;
#else
		m_IdealActivity = (Activity)(int)pTask->flData;;
#endif
		TaskComplete();
	...
	case TASK_GET_PATH_TO_ENEMY:
	{
		...
		if (IsRusher() && !HasConditions(bits_COND_CAN_RANGE_ATTACK1 | bits_COND_CAN_RANGE_ATTACK2))
		{
	...
	{
#ifdef GATESEQ
		m_actionActivity = GetSmallFlinchActivity();
#else
...
#ifdef GATESEQ
		m_actionActivity = GetDeathActivity(); // ku2zoff
#else
