...
void CBaseTurret::Precache()
{
	...
	PRECACHE_SOUND("turret/hks1.wav");
	PRECACHE_SOUND("turret/hks2.wav");
	PRECACHE_SOUND("turret/hks3.wav");
}
...
void CMiniTurret::Shoot(Vector& vecSrc, Vector& vecDirToEnemy)
{
	...
	{
	case 0:
		EMIT_SOUND(ENT(pev), CHAN_WEAPON, "turret/hks1.wav", 1, ATTN_NORM);
		break;
	case 1:
		EMIT_SOUND(ENT(pev), CHAN_WEAPON, "turret/hks2.wav", 1, ATTN_NORM);
		break;
	case 2:
		EMIT_SOUND(ENT(pev), CHAN_WEAPON, "turret/hks3.wav", 1, ATTN_NORM);
		break;
	}
	pev->effects = pev->effects | EF_MUZZLEFLASH;
}
...
int CBaseTurret::Classify()
{
	...
	{
		if ((pev->spawnflags & SF_MONSTER_TURRET_ALLY) != 0)
		{
