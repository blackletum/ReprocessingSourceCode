...
#define SF_BUTTON_USE_KEYCARD 128 // button need keycard to work
#define SF_BUTTON_TOUCH_ONLY 256  // button only fires as a result of USE key.
#define SF_BUTTON_NO_USE_THROUGH_WALL 512  // button only fires as a result of USE key.
...
void CBaseButton::ProcessChild(void)
{
	if (!m_pParent) return;

	CBaseEntity::ProcessChild();
...
void CBaseButton::ButtonTouch(CBaseEntity* pOther)
{
	...
	if (!UTIL_IsMasterTriggered(m_sMaster, pOther) || (FClassnameIs(pev, "func_button") &&
		 FBitSet(pev->spawnflags, SF_BUTTON_USE_KEYCARD) && pPlayer && pPlayer->m_rgItems[ITEM_SECURITY] == 0 ) )
	{
...
void CBaseButton::ButtonActivate()
{
	...
	CBaseEntity* pOther = m_hActivator;
	CBasePlayer* pPlayer = (CBasePlayer*)pOther;

	if (!UTIL_IsMasterTriggered(m_sMaster, m_hActivator) || (FClassnameIs(pev, "func_button") &&
		FBitSet(pev->spawnflags, SF_BUTTON_USE_KEYCARD) && pPlayer && pPlayer->m_rgItems[ITEM_SECURITY] == 0))
	{
	...
	if (FClassnameIs(pev, "func_button") && FBitSet(pev->spawnflags, SF_BUTTON_USE_KEYCARD) && pPlayer)
	{
		pPlayer->m_rgItems[ITEM_SECURITY]--;
		pev->spawnflags &= ~SF_BUTTON_USE_KEYCARD;
	}
