...
public:
	...
	void Touch(CBaseEntity* pOther) override
	{
		if (pOther->IsPlayer())
		{
			pOther->TakeDamage(pev, pev, gSkillData.leechDmgBite, DMG_POISON);
			// If the client is pushing me, give me some base velocity
			if (gpGlobals->trace_ent && gpGlobals->trace_ent == edict())
			{
				pev->basevelocity = pOther->pev->velocity;
	...
	void SetObjectCollisionBox() override
	{
		...
		pev->absmax = pev->origin + Vector(16, 16, 2);
	}
};

LINK_ENTITY_TO_CLASS(monster_smlsquid, CSmlSquid);

void CSmlSquid::Spawn()
{
	Precache();
	SET_MODEL(ENT(pev), "models/smlsquid.mdl");
	// Just for fun
	//	SET_MODEL(ENT(pev), "models/icky.mdl");
...
void CSmlSquid::Precache()
{
	//PRECACHE_MODEL("models/icky.mdl");
	PRECACHE_MODEL("models/smlsquid.mdl");

	PRECACHE_SOUND_ARRAY(pAttackSounds);
	PRECACHE_SOUND_ARRAY(pAlertSounds);
};

bool CSmlSquid::TakeDamage(entvars_t* pevInflictor, entvars_t* pevAttacker, float flDamage, int bitsDamageType)
{
	return CBaseMonster::TakeDamage(pevInflictor, pevAttacker, flDamage, bitsDamageType);
}