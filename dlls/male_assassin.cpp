...
#define MASSN_SNIPER_CLIP_SIZE			5
...
void CMOFAssassin::Killed(entvars_t* pevAttacker, int iGib)
{
	if (InSquad())
	{
...
// Crossbow
//=========================================================
void CMOFAssassin::Crossbow(void)
{
	if (m_hEnemy == NULL)
	{
		return;
	...
	if (FBitSet(pev->weapons, MAssassinWeaponFlag::SniperRifle) && gpGlobals->time - m_flLastShot <= 2.0)
	{
		return;
	}
...
void CMOFAssassin :: Shoot ()
{
	...
	else
	{
		...
		FireBullets( 1, vecShootOrigin, vecShootDir, VECTOR_CONE_1DEGREES, 2048, BULLET_MONSTER_MP5);
	}
...
Task_t	tlOFMAssassinCombatFail[] =
{
	{ TASK_STOP_MOVING,			0				},
	{ TASK_SET_ACTIVITY,		(float)ACT_IDLE },
	{ TASK_WAIT_FACE_ENEMY,		(float)0.2		},
	{ TASK_WAIT_PVS,			(float)0		},
};
	...
	case SCHED_TAKE_COVER_FROM_ENEMY:
		{
			if ( InSquad() )
			{
				if ( g_iSkillLevel >= SKILL_MEDIUM && HasConditions( bits_COND_CAN_RANGE_ATTACK2 ) && OccupySlot( bits_SLOTS_HGRUNT_GRENADE ) )
				{
...
class CFassassin : public CMOFAssassin
{
public:
	void Spawn() override;
	void Precache() override;
...
LINK_ENTITY_TO_CLASS(monster_female_assassin, CFassassin);

TYPEDESCRIPTION	CFassassin::m_SaveData[] =
{
	DEFINE_FIELD(CFassassin, m_flNextGrenadeCheck, FIELD_TIME),
	DEFINE_FIELD(CFassassin, m_flNextPainTime, FIELD_TIME),
		DEFINE_FIELD(CFassassin, m_vecTossVelocity, FIELD_VECTOR),
		DEFINE_FIELD(CFassassin, m_fThrowGrenade, FIELD_BOOLEAN),
		DEFINE_FIELD(CFassassin, m_fStanding, FIELD_BOOLEAN),
		DEFINE_FIELD(CFassassin, m_fFirstEncounter, FIELD_BOOLEAN),
		DEFINE_FIELD(CFassassin, m_cClipSize, FIELD_INTEGER),
		DEFINE_FIELD(CFassassin, m_voicePitch, FIELD_INTEGER),
			DEFINE_FIELD(CFassassin, m_iSentence, FIELD_INTEGER),
			DEFINE_FIELD(CFassassin, m_iTargetRanderamt, FIELD_INTEGER),
			DEFINE_FIELD(CFassassin, m_iAssassinHead, FIELD_INTEGER),
			DEFINE_FIELD(CFassassin, m_flLastShot, FIELD_TIME),
			DEFINE_FIELD(CFassassin, m_fStandingGround, FIELD_BOOLEAN),
			DEFINE_FIELD(CFassassin, m_flStandGroundRange, FIELD_FLOAT),
};

IMPLEMENT_SAVERESTORE(CFassassin, CSquadMonster);
