...
void SpawnBlood(Vector vecSpot, int bloodColor, float flDamage)
{
	...
	if (flDamage >= 40)
	{
		...
		{
			UTIL_BloodStream(vecSpot, UTIL_RandomBloodVector(), 71, RANDOM_LONG(flDamage - 30, flDamage + 30));
		}
		else
		{
			UTIL_BloodStream(vecSpot, UTIL_RandomBloodVector(), bloodColor, RANDOM_LONG(flDamage - 30, flDamage + 30));
		}
	}
}
...
bool CBasePlayerWeapon::AddPrimaryAmmo(CBasePlayerWeapon* origin, int iCount, char* szName, int iMaxClip, int iMaxCarry)
{
	...
	// in some weird cases weapon can lost m_pPlayer and other bad thing happen
	// To awoid the crash - we just delete the problematic weapon from player
	if (!m_pPlayer)
	{
		m_pPlayer = (CBasePlayer*)UTIL_GetLocalPlayer();
		m_pPlayer->RemovePlayerItem(this);
		m_pPlayer->GiveNamedItem(STRING(pev->classname));
	}
...
{
	DEFINE_FIELD(CPython, m_fSpotActive, FIELD_BOOLEAN),
};
...
TYPEDESCRIPTION CShotgun::m_SaveData[] =
	{
		...
		DEFINE_FIELD(CShotgun, m_flNextReload, FIELD_TIME),
		DEFINE_FIELD(CShotgun, m_bEmpty, FIELD_BOOLEAN),
