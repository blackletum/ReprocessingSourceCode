...
// Special for monsters and their specific shit
class CTriggerMonster : public CBaseDelay
{
public:
	bool KeyValue(KeyValueData* pkvd) override;
	void Spawn() override;
...
LINK_ENTITY_TO_CLASS(trigger_monster, CTriggerMonster);

TYPEDESCRIPTION CTriggerMonster::m_SaveData[] =
{
	DEFINE_FIELD(CTriggerMonster, m_iszMonster, FIELD_STRING),
	DEFINE_FIELD(CTriggerMonster, m_iSFlag, FIELD_INTEGER),
	DEFINE_FIELD(CTriggerMonster, m_iSFMode, FIELD_INTEGER),
	DEFINE_FIELD(CTriggerMonster, m_fRadius, FIELD_FLOAT),
	DEFINE_FIELD(CTriggerMonster, m_bMonFollow, FIELD_BOOLEAN),
};

IMPLEMENT_SAVERESTORE(CTriggerMonster, CBaseDelay);

bool CTriggerMonster::KeyValue(KeyValueData* pkvd)
{
	...
	else if (FStrEq(pkvd->szKeyName, "sflag"))
	{
		m_iSFlag = atoi(pkvd->szValue);
		return true;
	}
	else if (FStrEq(pkvd->szKeyName, "sfmode"))
	{
		m_iSFMode = atoi(pkvd->szValue);
		return true;
	}
	else if (FStrEq(pkvd->szKeyName, "forcefollow"))
	{
		m_bMonFollow = atoi(pkvd->szValue);
		return true;
	}
	else if (FStrEq(pkvd->szKeyName, "followradius"))
	{
...
void CTriggerMonster::Spawn()
{
}


void CTriggerMonster::Use(CBaseEntity* pActivator, CBaseEntity* pCaller, USE_TYPE useType, float value)
{
	edict_t* pentTarget;
	...
	while (!FNullEnt(pentTarget))
	{
		if (FBitSet(VARS(pentTarget)->flags, FL_MONSTER))
		{
			...
			if (pTarget && m_iSFlag && m_iSFMode)
			{
				ALERT(at_console, "SF swithcer %d %d\n", m_iSFlag, m_iSFMode);
				if (m_iSFMode == 1 && (pTarget->pev->spawnflags & m_iSFlag) != 0)
					pTarget->pev->spawnflags &= ~m_iSFlag;
				else if (m_iSFMode == 2 && (pTarget->pev->spawnflags & m_iSFlag) == 0)
					pTarget->pev->spawnflags |= m_iSFlag;
				else if (m_iSFMode == 3)
				{
					if ((pTarget->pev->spawnflags & m_iSFlag) != 0)
						pTarget->pev->spawnflags &= ~m_iSFlag;
					else if ((pTarget->pev->spawnflags & m_iSFlag) == 0)
						pTarget->pev->spawnflags |= m_iSFlag;
				}
			}
			if (pTarget && m_bMonFollow)
			{
				ALERT(at_console, "Follow swithcer\n");
				CTalkMonster* pFollow = (CTalkMonster*)GetMonsterPointer(pentTarget);
				if (pFollow->IsFollowing())
				{
					pFollow->StopFollowing(true);
				}
				else
				{
					CBaseEntity* pEntity = NULL;
					while ((pEntity = UTIL_FindEntityInSphere(pEntity, pev->origin, m_fRadius)) != NULL)
					{
						if (FClassnameIs(pEntity->pev, "player"))
						{
							if (pEntity->IsPlayer())
							{
								pFollow->StartFollowing(pEntity);
							}
						}
					}
				}
...
const char* GetCDTrack(int iTrack)
{
	switch (iTrack)
	{
	case 1:
...
void PlayCDTrack(int iTrack)
{
	...
	CBasePlayer* pPlayer = GetClassPtr((CBasePlayer*)VARS(pClient->edict()));
	pPlayer->m_iTrack = iTrack;
