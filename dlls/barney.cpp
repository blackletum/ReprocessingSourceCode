...
{
	if (GetBodygroup(BARNEY_GUN_GROUP) != BARNEY_BODY_GUNGONE)
	{ // drop the gun!
		Vector vecGunPos;
		Vector vecGunAngles;
...
//
//=========================================================
class CDeadBarney2 : public CBaseMonster
{
public:
	void Spawn() override;
	int Classify() override { return CLASS_PLAYER_ALLY; }
...
const char* CDeadBarney2::m_szPoses[] = {"lying_on_back", "lying_on_side", "lying_on_stomach"};

bool CDeadBarney2::KeyValue(KeyValueData* pkvd)
{
	if (FStrEq(pkvd->szKeyName, "pose"))
	{
		m_iPose = atoi(pkvd->szValue);
...
LINK_ENTITY_TO_CLASS(monster_nakedbarney_dead, CDeadBarney2);

//=========================================================
// ********** DeadBarney2 SPAWN **********
//=========================================================
void CDeadBarney2::Spawn()
{
...
// Returns number of events handled, 0 if none.
//=========================================================
void CEvilBarney::HandleAnimEvent(MonsterEvent_t* pEvent)
{
	switch (pEvent->event)
	{
	case BARNEY_AE_SHOOT:
...
// the enemy barney is facing.
//=========================================================
void CEvilBarney::AgentFirePistol()
{
	Vector vecShootOrigin;
	...
	FireBullets(1, vecShootOrigin, vecShootDir, VECTOR_CONE_5DEGREES, 1024, BULLET_MONSTER_357);
	...
	if (pitchShift > 10)
		...
		pitchShift -= 5;
	EMIT_SOUND_DYN(ENT(pev), CHAN_WEAPON, "weapons/357_shot2.wav", 1, ATTN_NORM, 0, 100 + pitchShift);
...
bool CEvilBarney::TakeDamage(entvars_t* pevInflictor, entvars_t* pevAttacker, float flDamage, int bitsDamageType)
{
	// make sure friends talk about it if player hurts talkmonsters...
	bool ret = CTalkMonster::TakeDamage(pevInflictor, pevAttacker, flDamage, bitsDamageType);
...
public:
	void Spawn() override;
	int Classify() override { return m_iClassify; }
...
bool CDeadMonster::KeyValue(KeyValueData* pkvd)
{
	...
	else if (FStrEq(pkvd->szKeyName, "framenum"))
	{
		m_iframe = atoi(pkvd->szValue);
		return true;
	}
	else if (FStrEq(pkvd->szKeyName, "blood"))
	{
		int iBlood = atoi(pkvd->szValue);
		if (iBlood == 1)
			m_iBlood = BLOOD_COLOR_YELLOW;
		return true;
	}
	else if (FStrEq(pkvd->szKeyName, "gib"))
	{
		int iClass = atoi(pkvd->szValue);
		if (iClass == 1)
		{
			m_iClassify = CLASS_ALIEN_MILITARY;
			m_iGib = 1;
		}
		else if (iClass == 2)
		{
...
LINK_ENTITY_TO_CLASS(monster_dead, CDeadMonster);

//=========================================================
// ********** DeadBarney SPAWN **********
//=========================================================
void CDeadMonster::Spawn()
{
	if (pev->model)
	{
		PRECACHE_MODEL((char*)STRING(pev->model)); //LRC
	...
	if (pev->sequence == -1)
	{
		ALERT(at_console, "Monster with bad pose\n");
	}
	// Corpses have less health
	pev->health = 8; //gSkillData.barneyHealth;
