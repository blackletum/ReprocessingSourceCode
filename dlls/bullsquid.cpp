...
void CBullsquid::HandleAnimEvent(MonsterEvent_t* pEvent)
{
	...
	case BSQUID_AE_SPIT:
	{
		if (m_hEnemy)
		{
			...
			vecSpitDir.x -= RANDOM_FLOAT(-0.25, 0.25);
			vecSpitDir.y -= RANDOM_FLOAT(-0.25, 0.25);
			vecSpitDir.z -= RANDOM_FLOAT(-0.25, 0);

			CSquidSpit::Shoot(pev, vecSpitOffset, vecSpitDir * 750);

			vecSpitDir = ((m_hEnemy->pev->origin + m_hEnemy->pev->view_ofs) - vecSpitOffset).Normalize();

			vecSpitDir.x += RANDOM_FLOAT(-0.15, 0.35);
			vecSpitDir.y += RANDOM_FLOAT(-0.15, 0.35);
			vecSpitDir.z += RANDOM_FLOAT(-0.15, 0);
...
class CPacksquid : public CBullsquid
{
public:
	void Spawn() override;
	void Precache() override;
...
LINK_ENTITY_TO_CLASS(monster_packsquid, CPacksquid);

int CPacksquid::Classify()
{
...
// Spawn
//=========================================================
void CPacksquid::Spawn()
{
	Precache();

	SET_MODEL(ENT(pev), "models/packsquid.mdl");
	UTIL_SetSize(pev, Vector(-32, -32, 0), Vector(32, 32, 64));
	pev->solid = SOLID_SLIDEBOX;
	pev->movetype = MOVETYPE_STEP;
	m_bloodColor = BLOOD_COLOR_GREEN;
	pev->effects = 0;
	pev->health = gSkillData.bullsquidHealth*1;
	m_flFieldOfView = 0.2; // indicates the width of this monster's forward view cone ( as a dotproduct result )
	m_MonsterState = MONSTERSTATE_NONE;
...
// Precache - precaches all resources this monster needs
//=========================================================
void CPacksquid::Precache()
{
	PRECACHE_MODEL("models/packsquid.mdl");
...
//////////////////////////////////////
// MOMSQUID - Она же мамака
//////////////////////////////////////

class CMomsquid : public CBullsquid
{
public:
	void Spawn() override;
	void Precache() override;
...
LINK_ENTITY_TO_CLASS(monster_momsquid, CMomsquid);

int CMomsquid::Classify()
{
	return CLASS_ALIEN_MONSTER;
}
...
// Spawn
//=========================================================
void CMomsquid::Spawn()
{
	Precache();

	SET_MODEL(ENT(pev), "models/momsquid.mdl");
	UTIL_SetSize(pev, Vector(-32, -32, 0), Vector(32, 32, 64));
	pev->solid = SOLID_SLIDEBOX;
	pev->movetype = MOVETYPE_STEP;
	m_bloodColor = BLOOD_COLOR_GREEN;
...
// that occur when tagged animation frames are played.
//=========================================================
void CMomsquid::HandleAnimEvent(MonsterEvent_t* pEvent)
{
	switch (pEvent->event)
	{
	case BSQUID_AE_SPIT:
	{
		if (m_hEnemy)
		{
			...
			int rand = RANDOM_FLOAT(0, 100);
			if (rand < 21)
			{
				CBaseEntity* pChild = CBaseEntity::Create("monster_smlsquid", pev->origin, pev->angles, edict());
			...
			CSquidSpit::Shoot(pev, vecSpitOffset, vecSpitDir * 1500);
			...
			CSquidSpit::Shoot(pev, vecSpitOffset, vecSpitDir * 1550);
			...
			CSquidSpit::Shoot(pev, vecSpitOffset, vecSpitDir * 1400);
		}
	}
	break;
	...
	{
		// SOUND HERE!
		CBaseEntity* pHurt = CheckTraceHullAttack(70, gSkillData.bullsquidDmgBite*2, DMG_SLASH);
	...
	case BSQUID_AE_TAILWHIP:
	{
		CBaseEntity* pHurt = CheckTraceHullAttack(70, gSkillData.bullsquidDmgWhip * 2, DMG_CLUB | DMG_ALWAYSGIB);
		if (pHurt)
		{
			pHurt->pev->punchangle.z = -20;
			pHurt->pev->punchangle.x = 20;
...
// Precache - precaches all resources this monster needs
//=========================================================
void CMomsquid::Precache()
{
	PRECACHE_MODEL("models/momsquid.mdl");
	...
	UTIL_PrecacheOther("monster_smlsquid");
}