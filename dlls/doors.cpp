...
void CBaseDoor::Use(CBaseEntity* pActivator, CBaseEntity* pCaller, USE_TYPE useType, float value)
{
	...
	// Freeslave inspired idea of fix monsters and doors
	// UNDONE need to rewrite doors logic
	// end of idea
...
bool CBaseDoor::DoorActivate()
{
	...
	{ // door should open
		...
		if (m_hActivator != NULL && m_hActivator->IsPlayer() && m_bHealthValue)
		...
		if (!heal)
		{
			// Awful hack, but i dont care. It's works and nothing else metter
...
//
// Starts the door going to its "down" position (simply ToggleData->vecPosition1), but without fire target
//
void CBaseDoor::DoorReset()
{
	if (!FBitSet(pev->spawnflags, SF_DOOR_SILENT))
	{
		if (m_toggle_state != TS_GOING_UP && m_toggle_state != TS_GOING_DOWN)
	...
	SetMoveDone(&CBaseDoor::DoorHitBottomNoFire);
	if (FClassnameIs(pev, "func_door_rotating")) //rotating door
		AngularMove(m_vecAngle1, pev->speed);
	else
		LinearMove(m_vecPosition1, pev->speed);
...
//
void CBaseDoor::DoorHitBottomNoFire()
{
	if (!FBitSet(pev->spawnflags, SF_DOOR_SILENT))
	{
		STOP_SOUND(ENT(pev), CHAN_STATIC, (char*)STRING(pev->noiseMoving));
			...
			if (VARS(pentTarget) != pev)
			{
				...
				if (FClassnameIs(pentTarget, "func_door") || FClassnameIs(pentTarget, "func_door_rotating"))
				{
					...
					if (pDoor->m_flWait >= 0)
					{
						...
						if (pDoor->pev->movedir == pev->movedir && pDoor->pev->velocity == pev->velocity && pDoor->pev->avelocity == pev->avelocity)
						{
