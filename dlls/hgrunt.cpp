...
bool CHGrunt::CheckRangeAttack1(float flDot, float flDist)
{
	...
	if (HasConditions(bits_COND_ENEMY_OCCLUDED) && flDist <= 4096 && flDot >= 0.5 && NoFriendlyFire() && FBitSet(pev->weapons, HGRUNT_RPG))
	{
		if (flDist <= 64)
		{
			// fuck
...
{
	if (!FBitSet(pev->weapons, (HGRUNT_HANDGRENADE | HGRUNT_GRENADELAUNCHER)) || FBitSet(pev->weapons, HGRUNT_RPG))
	{
		return false;
	}
...
void CHGrunt::ShootM16()
{
	...
	Vector vecShellVelocity = gpGlobals->v_right * RANDOM_FLOAT(40, 90) + gpGlobals->v_up * RANDOM_FLOAT(75, 200) + gpGlobals->v_forward * RANDOM_FLOAT(-40, 40);
	EjectBrass(vecShootOrigin - vecShootDir * 24, vecShellVelocity, pev->angles.y, m_iRifleShell, TE_BOUNCE_SHELL);
	FireBullets(1, vecShootOrigin, vecShootDir, VECTOR_CONE_10DEGREES, 2048, BULLET_MONSTER_MP5); // shoot +-5 degrees
...
void CHGrunt::RPG()
{
	...
	CBaseEntity* pRocket = CBaseEntity::Create("rpg_rocket", vecShootOrigin, angDir, edict());
	//if (pRocket)
		//pRocket->pev->velocity = m_vecTossVelocity;

	//pev->effects |= EF_MUZZLEFLASH;

	MESSAGE_BEGIN(MSG_PVS, SVC_TEMPENTITY, vecShootDir);
	WRITE_BYTE(TE_SMOKE);
...
void CHGrunt::HandleAnimEvent(MonsterEvent_t* pEvent)
{
	...
	case HGRUNT_AE_DROP_GUN:
	{
		if (GetBodygroup(GUN_GROUP) != GUN_NONE)
		{
			...
			{
				DropItem("weapon_m16", vecGunPos, vecGunAngles);
			}
			else
			{
...
void CHGrunt::Spawn()
{
	...
	{
		SetBodygroup(GUN_GROUP, GUN_M16);
	...
	if (RANDOM_LONG(0, 99) < 80)
		pev->skin = RANDOM_FLOAT(0, 5); // random skins

	if (GetBodygroup(1) == HEAD_COMMANDER)
		pev->skin = RANDOM_FLOAT(0, 5); // HACK! random skins for commander
...
void CHGrunt::Precache()
{
	...
	PRECACHE_SOUND("hgrunt/m162.wav");
...
void CDeadHGrunt::Spawn()
{
	...
	case 4: // Medic no Gun
		pev->body = 0;
		pev->skin = 0;
		SetBodygroup(HEAD_GROUP, HEAD_MEDIC);
		SetBodygroup(GUN_GROUP, GUN_NONE);
		break;
	case 5: // M203 no Gun
		pev->body = 0;
		...
		SetBodygroup(GUN_GROUP, GUN_NONE);
		break;
	case 6: // No head no Gun
		pev->body = 0;
		pev->skin = 0;
		SetBodygroup(HEAD_GROUP, HEAD_SHOT);
		SetBodygroup(GUN_GROUP, GUN_NONE);
		break;
	}
...
// DEAD HSARGE PROP
//=========================================================
class CDeadHSarge : public CBaseMonster
{
public:
	void Spawn() override;
	int Classify() override { return CLASS_HUMAN_MILITARY; }
...
const char* CDeadHSarge::m_szPoses[] = {"deadstomach", "deadside", "deadsitting"};

bool CDeadHSarge::KeyValue(KeyValueData* pkvd)
{
	if (FStrEq(pkvd->szKeyName, "pose"))
	{
		m_iPose = atoi(pkvd->szValue);
...
LINK_ENTITY_TO_CLASS(monster_hsarge_dead, CDeadHSarge);

//=========================================================
// ********** DeadHGrunt SPAWN **********
//=========================================================
void CDeadHSarge::Spawn()
{
	PRECACHE_MODEL("models/hsarge.mdl");
	SET_MODEL(ENT(pev), "models/hsarge.mdl");
...
class CNamGrunt : public CHGrunt
{
public:
	void Spawn() override;
	void Precache() override;
...
// Spawn
//=========================================================
void CNamGrunt::Spawn()
{
	Precache();
	if (pev->model)
		SET_MODEL(ENT(pev), STRING(pev->model));
	else
		SET_MODEL(ENT(pev), "models/hallucination1.mdl");
	UTIL_SetSize(pev, VEC_HUMAN_HULL_MIN, VEC_HUMAN_HULL_MAX);
...
// Precache - precaches all resources this monster needs
//=========================================================
void CNamGrunt::Precache()
{
	if (pev->model)
		PRECACHE_MODEL((char*)STRING(pev->model));
	else
		PRECACHE_MODEL("models/hallucination1.mdl");
