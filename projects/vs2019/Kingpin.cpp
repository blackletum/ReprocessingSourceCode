...
{
	DEFINE_FIELD(CKingpin, m_flForce, FIELD_FLOAT),
	DEFINE_FIELD(CKingpin, m_flNextPainSound, FIELD_TIME),
	DEFINE_FIELD(CKingpin, m_velocity, FIELD_VECTOR),
	DEFINE_FIELD(CKingpin, m_avelocity, FIELD_VECTOR),
	DEFINE_FIELD(CKingpin, m_vecTarget, FIELD_VECTOR),
	DEFINE_FIELD(CKingpin, m_posTarget, FIELD_POSITION_VECTOR),
	DEFINE_FIELD(CKingpin, m_vecDesired, FIELD_VECTOR),
	DEFINE_FIELD(CKingpin, m_posDesired, FIELD_POSITION_VECTOR),
	DEFINE_FIELD(CKingpin, m_flMinZ, FIELD_FLOAT),
	DEFINE_FIELD(CKingpin, m_flMaxZ, FIELD_FLOAT),
	DEFINE_FIELD(CKingpin, m_vecGoal, FIELD_VECTOR),
	DEFINE_FIELD(CKingpin, m_flLastSeen, FIELD_TIME),
	DEFINE_FIELD(CKingpin, m_flPrevSeen, FIELD_TIME),
	DEFINE_FIELD(CKingpin, m_irritation, FIELD_INTEGER),
	DEFINE_FIELD(CKingpin, m_iLevel, FIELD_INTEGER),
	DEFINE_FIELD(CKingpin, m_iTeleport, FIELD_INTEGER),
	DEFINE_FIELD(CKingpin, m_hRecharger, FIELD_EHANDLE),
	DEFINE_ARRAY(CKingpin, m_hSphere, FIELD_EHANDLE, N_SPHERES),
	DEFINE_FIELD(CKingpin, m_iActiveSpheres, FIELD_INTEGER),
	DEFINE_FIELD(CKingpin, m_flAdj, FIELD_FLOAT),
	DEFINE_FIELD(CKingpin, m_pBall, FIELD_CLASSPTR),
	DEFINE_ARRAY(CKingpin, m_szRechargerTarget, FIELD_CHARACTER, 64),
	DEFINE_ARRAY(CKingpin, m_szDrawUse, FIELD_CHARACTER, 64),
	DEFINE_ARRAY(CKingpin, m_szTeleportUse, FIELD_CHARACTER, 64),
	DEFINE_ARRAY(CKingpin, m_szTeleportTouch, FIELD_CHARACTER, 64),
	DEFINE_ARRAY(CKingpin, m_szDeadUse, FIELD_CHARACTER, 64),
	DEFINE_ARRAY(CKingpin, m_szDeadTouch, FIELD_CHARACTER, 64),
	DEFINE_FIELD(CKingpin, m_flShootEnd, FIELD_TIME),
	DEFINE_FIELD(CKingpin, m_flShootTime, FIELD_TIME),
	DEFINE_ARRAY(CKingpin, m_hFriend, FIELD_EHANDLE, 3),
};

IMPLEMENT_SAVERESTORE(CKingpin, CBaseMonster);

class CKingpinHVR : public CBaseMonster
{
public:
	bool Save(CSave& save) override;
	bool Restore(CRestore& restore) override;
	...
	void AbsorbInit();
	void TeleportInit(CKingpin* pOwner, CBaseEntity* pEnemy, CBaseEntity* pTarget, CBaseEntity* pTouch);
	void GreenBallInit();
	...
	float m_flIdealVel;
	Vector m_vecIdeal;
	CKingpin* m_pNihilanth;
	EHANDLE m_hTouch;
	int m_nFrames;
};

LINK_ENTITY_TO_CLASS(kingpin_energy_ball, CKingpinHVR);


TYPEDESCRIPTION CKingpinHVR::m_SaveData[] =
{
	DEFINE_FIELD(CKingpinHVR, m_flIdealVel, FIELD_FLOAT),
	DEFINE_FIELD(CKingpinHVR, m_vecIdeal, FIELD_VECTOR),
	DEFINE_FIELD(CKingpinHVR, m_pNihilanth, FIELD_CLASSPTR),
	DEFINE_FIELD(CKingpinHVR, m_hTouch, FIELD_EHANDLE),
	DEFINE_FIELD(CKingpinHVR, m_nFrames, FIELD_INTEGER),
};


IMPLEMENT_SAVERESTORE(CKingpinHVR, CBaseMonster);
...
const char* CKingpin::pBallSounds[] =
{
	"X/x_ballattack1.wav",
};

const char* CKingpin::pShootSounds[] =
{
	"X/x_shoot1.wav",
};

const char* CKingpin::pRechargeSounds[] =
{
	"X/x_recharge1.wav",
	"X/x_recharge2.wav",
	"X/x_recharge3.wav",
...
const char* CKingpin::pLaughSounds[] =
{
	"X/x_laugh1.wav",
	"X/x_laugh2.wav",
};
...
void CKingpin::Spawn()
{
	...
	SET_MODEL(edict(), "models/kingpin.mdl");
	...
	SetThink(&CKingpin::StartupThink);
...
void CKingpin::StartupUse(CBaseEntity* pActivator, CBaseEntity* pCaller, USE_TYPE useType, float value)
{
	SetThink(&CKingpin::HuntThink);
	pev->nextthink = gpGlobals->time + 0.1;
	SetUse(&CKingpin::CommandUse);
}


void CKingpin::StartupThink()
{
	m_irritation = 0;
	m_flAdj = 512;
...
void CKingpin::DyingThink()
{
	...
	CKingpinHVR* pEntity = (CKingpinHVR*)Create("kingpin_energy_ball", vecSrc, pev->angles, edict());
...
void CKingpin::CrashTouch(CBaseEntity* pOther)
{
...
void CKingpin::FloatSequence()
{
}


void CKingpin::ShootBalls()
{
	if (m_flShootEnd > gpGlobals->time)
	{
		...
		while (m_flShootTime < m_flShootEnd && m_flShootTime < gpGlobals->time)
		{
			if (m_hEnemy != NULL)
			{
				...
				pEntity = (CKingpinHVR*)Create("kingpin_energy_ball", vecSrc, pev->angles, edict());
...
void CKingpin::MakeFriend(Vector vecStart)
{
	int i;
...
bool CKingpin::AbsorbSphere()
{
	for (int i = 0; i < N_SPHERES; i++)
	{
		if (m_hSphere[i] != NULL)
		{
			CKingpinHVR* pSphere = (CKingpinHVR*)((CBaseEntity*)m_hSphere[i]);
			pSphere->AbsorbInit();
			m_hSphere[i] = NULL;
			m_iActiveSpheres--;
			return true;
...
void CKingpin::TargetSphere(USE_TYPE useType, float value)
{
...
void CKingpin::CommandUse(CBaseEntity* pActivator, CBaseEntity* pCaller, USE_TYPE useType, float value)
{
	switch (useType)
	{
	case USE_OFF:
...
CBaseEntity* CKingpin::RandomTargetname(const char* szName)
{
	int total = 0;
...
void CKingpinHVR::CircleInit(CBaseEntity* pTarget)
{
	...
	SetThink(&CKingpinHVR::HoverThink);
	SetTouch(&CKingpinHVR::BounceTouch);
	pev->nextthink = gpGlobals->time + 0.1;
...
CBaseEntity* CKingpinHVR::RandomClassname(const char* szName)
{
	int total = 0;
...
void CKingpinHVR::ZapInit(CBaseEntity* pEnemy)
{
	...
	m_hEnemy = pEnemy;
	SetThink(&CKingpinHVR::ZapThink);
	SetTouch(&CKingpinHVR::ZapTouch);
	pev->nextthink = gpGlobals->time + 0.1;

	EMIT_SOUND_DYN(edict(), CHAN_WEAPON, "debris/zap4.wav", 1, ATTN_NORM, 0, 100);
}

void CKingpinHVR::ZapThink()
{
...
void CKingpinHVR::ZapTouch(CBaseEntity* pOther)
{
...
void CKingpinHVR::TeleportInit(CKingpin* pOwner, CBaseEntity* pEnemy, CBaseEntity* pTarget, CBaseEntity* pTouch)
{
	pev->movetype = MOVETYPE_FLY;
	pev->solid = SOLID_BBOX;
	...
	SetThink(&CKingpinHVR::TeleportThink);
	SetTouch(&CKingpinHVR::TeleportTouch);
	pev->nextthink = gpGlobals->time + 0.1;
...
void CKingpinHVR::GreenBallInit()
{
	pev->movetype = MOVETYPE_FLY;
	pev->solid = SOLID_BBOX;
	...
	SetTouch(&CKingpinHVR::RemoveTouch);
}


void CKingpinHVR::TeleportThink()
{
	pev->nextthink = gpGlobals->time + 0.1;
...
void CKingpinHVR::AbsorbInit()
{
	SetThink(&CKingpinHVR::DissipateThink);
	pev->renderamt = 255;
...
void CKingpinHVR::TeleportTouch(CBaseEntity* pOther)
{
...
void CKingpinHVR::DissipateThink()
{
	pev->nextthink = gpGlobals->time + 0.1;
...
bool CKingpinHVR::CircleTarget(Vector vecTarget)
{
	bool fClose = false;
...
void CKingpinHVR::MovetoTarget(Vector vecTarget)
{
	if (m_vecIdeal == Vector(0, 0, 0))
	{
		m_vecIdeal = pev->velocity;
...
void CKingpinHVR::RemoveTouch(CBaseEntity* pOther)
{
	STOP_SOUND(edict(), CHAN_WEAPON, "x/x_teleattack1.wav");
	UTIL_Remove(this);
}

void CKingpinHVR::BounceTouch(CBaseEntity* pOther)
{
